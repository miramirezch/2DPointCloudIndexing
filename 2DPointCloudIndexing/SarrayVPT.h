...
template<typename T, double(*distance)(const std::pair<sdsl::sd_vector<>, int>&, const std::pair<sdsl::sd_vector<>, int>&)>
class SarrayVPT
{
	using PointIdx = std::pair<sdsl::sd_vector<>, int>;
...
public:
	...
	SarrayVPT(std::string name, int cmax, int delta) :name_{ name }, cmax_{ cmax }, delta_{ delta } {}
	...
	sdsl::sd_vector<> GenerateSarray(const Cloud<T>& pointCloud) const
	{
		...
		for (const auto& point : pointCloud.Points)
		{
			...
			position = px + static_cast<int>(cmax_ / delta_)*py;
	...
	std::vector<std::pair<int, double>> KNN(const Cloud<T>& queryCloud, const int k, const int internalK) const
	{
