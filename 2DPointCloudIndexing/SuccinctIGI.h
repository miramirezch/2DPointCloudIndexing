...
{
private:
	std::unordered_map<int, sdsl::sd_vector<>> succinctIGI;
	std::unordered_map<int, int> sizeClouds;
	std::unordered_map<int, int> onesPerBitmap;
...
public:
	...
	SuccinctIGI(const std::vector<Cloud<T>>& pointClouds, std::string name, const int cmax, const int delta) :name_{ name }, cmax_{ cmax }, delta_{ delta }
	{
		std::unordered_map<int, std::set<int>> tempIGI;
	...
	void Add(const Cloud<T>& pointCloud, std::unordered_map<int, std::set<int>>& tempIGI)
	{
		int px, py, cell;
	...
	std::vector<std::pair<int, int>> KNN(const Cloud<T>& queryCloud, const int k) const
	{
		...
		std::unordered_map<int, sdsl::sd_vector<>> listSarrays;
