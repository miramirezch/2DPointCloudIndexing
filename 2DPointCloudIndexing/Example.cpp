...
using PointIdx = std::pair<Point, int>;
...
auto DistL22 = [](const Point& p1, const Point& p2)
{
	auto dx = std::pow(boost::geometry::get<0>(p1) - boost::geometry::get<0>(p2), 2);
	auto dy = std::pow(boost::geometry::get<1>(p1) - boost::geometry::get<1>(p2), 2);
...
auto DiscreteDistL2 = [](const PointIdx& p1, const PointIdx& p2)
{
	...
	return static_cast<int>(std::floor(std::sqrt(dx + dy)));
};

auto DiscreteDistL22 = [](const Point& p1, const Point& p2)
{
	auto dx = std::pow(boost::geometry::get<0>(p1) - boost::geometry::get<0>(p2), 2);
	auto dy = std::pow(boost::geometry::get<1>(p1) - boost::geometry::get<1>(p2), 2);
...
int main()
{
	...
	//// VPT
	VPT<Point> vpt2("VPT");
	vpt2.Build(cloudsIndexing, DistL22);

	//// BKT
	//BKT<Point> bkt2("BKT");
	//bkt2.Build(cloudsIndexing, DiscreteDistL22);

	//// IGIRtree
	//IGIRtree<Point> igiRtree2(cloudsIndexing, "IGIRtree", 10000, 10);

	//// IGIVpt
	//IGIVpt<Point> igiVPT2(cloudsIndexing, DistL22, "IGIVpt", 10000, 10);

	// RevLC
	//RevLC<Point> lc("List of Clusters");
	//lc.Build(cloudsIndexing, DistL22, 1000);
